...
define(['components/base', 'components/slideshow/assets','owl_carousel'], function(ComponentBase, assets) {
    ...
    SlideshowApp.ConfigView = ComponentBase.ConfigView.extend({
        ...
        _onRender: function() {
            ...
            McMore.uploadImageCallback = function(result){
                $('.image-upload.selected img').attr('src', result.url);
                console.log(result);
                self.updateSlideData()
            }
        ...
        addItem:function(){
            $('<li class="slide clearfix"><div class="delete-item"><img src="/marionette/images/delete_item.png"/></div><div class="slide-img image-upload" data-name="slide"><img src="/marionette/images/slideshow.png"/></div><div class="slide-name"><input type="text" name="slide_name" value=""/></div><div class="slide-link"><input type="text" name="slide_link" value=""/></div></li>').appendTo($('#slides-list ul'))
